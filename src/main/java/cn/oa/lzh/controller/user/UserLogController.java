...
import cn.oa.lzh.model.dao.taskdao.TaskDao;
...
import cn.oa.lzh.model.dao.user.UserLogRecordService;
import cn.oa.lzh.model.dao.user.UserLogService;
...
public class UserLogController {
		...
		for (User user : uList) {
			if (taskDao.countfinish(7l, user.getUserId()) > 0) {
				hashMap.put(user.getUserName(),
						taskDao.countfinish(7l, user.getUserId()));
				i++;
			}
		}
    ...
    //获得用户管理-》在线用户页面表格
	public void getuserlogrecord(int page, HttpSession session, Model model,
			String basekey, String time, String icon) {
		long userid = Long.valueOf(session.getAttribute("userId") + "");
		setTwo(model, basekey, time, icon);
		Page<LoginRecord> page4 = userLogRecordService.ulogpaging(page,
				basekey, userid, time);
	...
	public void getuserlog(int page, HttpSession session, Model model,
			String basekey, String time, String icon) {
		long userid = Long.valueOf(session.getAttribute("userId") + "");
		setTwo(model, basekey, time, icon);
		Page<UserLog> page3 = userLogService.ulogpaging(page, basekey, userid,
				time);
