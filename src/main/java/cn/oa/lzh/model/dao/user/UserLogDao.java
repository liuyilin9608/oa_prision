...
public interface UserLogDao extends JpaRepository<UserLog, Long>{
	...
	//查找出最新一条记录
	@Query(nativeQuery=true,value=("SELECT * from user_log where user_log.user_id=?1 ORDER BY user_log.log_time DESC LIMIT 0,1"))
	UserLog findByUserlaset(long userid);
	
	//限制13条
	@Query(nativeQuery=true,value=("SELECT * from user_log where user_log.user_id=?1 ORDER BY user_log.log_time DESC LIMIT 0,13"))
