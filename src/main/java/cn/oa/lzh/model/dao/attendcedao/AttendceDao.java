...
public interface AttendceDao extends JpaRepository<Attends, Long> {
	...
	Integer updateatttime(Date date, String hourmin, Long statusIdlong,
			long attid);
	...
	//查找当天上班签到的时间
	@Query("from Attends a where a.typeId=?1 and a.attendsTime=?2 ")
	Attends findByTypeAndDate(Long typeId, Date date);
	
	// 查找某用户当天下班的考勤记录id
	@Query(nativeQuery = true, value = "select a.attends_id from attends a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_user_id=?2 and a.type_id=9 ")
	Long findoffworkid(String date, long userid);

	// 查找某用户某天总共的记录
	@Query(nativeQuery = true, value = "SELECT COUNT(*) from attends a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_user_id=?2 ")
	Integer countrecord(String date, long userid);
	
	// 查找某用户某天总共的记录
		@Query(nativeQuery = true, value = "SELECT COUNT(*) from attends a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_user_id=?2 and a.type_id = 8 ")
		Integer countuprecord(String date, long userid);
		
		// 查找某用户某天总共的记录
		@Query(nativeQuery = true, value = "SELECT COUNT(*) from attends a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_user_id=?2 and a.type_id = 9 ")
		Integer countdownrecord(String date, long userid);
	

	// 查找某用户某天最新记录用来显示用户最新的类型和考勤时间
	@Query(nativeQuery = true, value = "SELECT * from attends a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_user_id=?2 ORDER  BY a.attends_time DESC  LIMIT 1")
	Attends findlastest(String date, long userid);
	
	// 查找某用户某天最新记录用来显示用户最新的类型和考勤时间
	@Query(nativeQuery = true, value = "SELECT * from attends a WHERE  a.attends_user_id=?1 ORDER  BY a.attends_time DESC  LIMIT 1")
	Attends findlast(long userid);

	/**
	 * 前一天的签到数
	 */
	@Query(nativeQuery = true, value = "SELECT count(*) from attends a WHERE DATE_format(a.attends_time,'%Y-%m-%d') like %?1% and a.attends_user_id=?2 ORDER  BY a.attends_time DESC  LIMIT 1")
	int findlastday(String date, long userid);
	
	
	@Query("from Attends a where a.user.userId=:userId ORDER BY a.attendsTime DESC")
	Page<Attends> findByUserOrderByAttendsTimeDesc(
			@Param("userId") long userid, Pageable pa);
	...
	Page<Attends> findByUserOrderByAttendsTimeDesc(
			@Param("ids") List<Long> user, Pageable pa);
	...
	Page<Attends> findByUserOrderByTypeIdDesc(@Param("ids") List<Long> user,
			Pageable pa);

	// 通过类型升序排序
	@Query("from Attends a where a.user.userId in (:ids)  ORDER BY a.typeId ASC ")
	Page<Attends> findByUserOrderByTypeIdAsc(@Param("ids") List<Long> user,
			Pageable pa);

	// 状态
	// 通过状态降序排序
	@Query("from Attends a where a.user.userId in (:ids)  ORDER BY a.statusId DESC ")
	Page<Attends> findByUserOrderByStatusIdDesc(@Param("ids") List<Long> user,
			Pageable pa);

	// 通过状态升序排序
	@Query("from Attends a where a.user.userId in (:ids)  ORDER BY a.statusId ASC ")
	Page<Attends> findByUserOrderByStatusIdAsc(@Param("ids") List<Long> user,
			Pageable pa);
	...
	Page<Attends> findByUserOrderByAttendsTimeAsc(
			@Param("ids") List<Long> user, Pageable pa);
